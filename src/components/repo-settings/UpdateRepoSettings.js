...
class initForm extends React.Component {
  render() {
    ...
    return (
      <Form onSubmit={this._updateRepository} layout="vertical">
        <Form.Item label="Repository name">
          {getFieldDecorator("name", {
            ...
            <Input
              style={{ width: "100%" }}
              prefix={
                <Icon type="snippets" style={{ color: "rgba(0,0,0,.25)" }} />
              }
        ...
        <Form.Item>
          ...
          <Button type="danger" icon="delete" onClick={this._showDeleteConfirm}>
    ...
    Modal.confirm({
      ...
      content: (
        <p>
          ...
          <b>{repository.name}</b> repository data. You should download a backup
          file.
        </p>
      ),
      okText: "Yes",
