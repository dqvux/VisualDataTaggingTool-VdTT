...
import { Tag, Input, Tooltip, Icon, Modal, notification, Affix, List, Menu } from 'antd'
...
export default class Tags extends React.Component {
    ...
    render() {
        ...
        return (
            <React.Fragment>
                <Affix style={{ position: 'fixed', bottom: 0, backgroundColor: '#e9ebee', width: '100%', padding: '10px 10px' }}>
                    ...
                    <div className='list-tags'>
                        {tags.map((tag) => {
                            ...
                            const tagElem = (
                                <Tag className='tag cursor-pointer' color={colors[tags.indexOf(tag)]} closable={true} onClose={(e) => this.handleClose(e, tag)} onClick={() => this.props._addTag(tag)}>
    ...
    _saveTags = async () => {
        ...
        try {
            ...
            if (updateds > 0) {
                notification.success({ message: 'Labels have been saved successfully' })
            } else {
...
class ShortKey extends React.Component {
    ...
    _handleShortKey = (event) => {
        ...
        if (this._textIsSelected()) { // check text is selected or not
            event.preventDefault()
            const key = event.key
            console.log(key)
            const candidateTags = tags.filter(tag => tag.toLowerCase().startsWith(key))
                                            ...
                                            candidateTags.map(item => (
                                                <Menu.Item key={item}
                                                    style={{
                                                        ...
                                                        backgroundColor: item === tag ? `${colors[tags.indexOf(tag)]}` : '',
