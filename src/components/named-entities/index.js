...
export default class NamedEntities extends React.Component {
    ...
    _addTag = (tag) => {
        ...
        if (textarea) {
            ...
            const sel = textarea.value.substring(start, end).replace(/ +/g, ' ').trim() // remove all unexpected space, use regex /\s+/g instead if wanna remove all white space such as: tab, newline, ...
            let replace = ''
            if (tokenDelimiter === 'space')
                replace = ` <${tag}>${sel}</${tag}> `
            else 
                replace = `<${tag}>${sel}</${tag}>`
            window.pattern = sel
    ...
    _getRepositoryData = async () => {
        ...
        try {
            ...
            this.setState({ tags, tokenDelimiter: repositories[0].settings.tokenDelimiter })
        } catch (ex) {
            console.error(ex)
        }
    }
