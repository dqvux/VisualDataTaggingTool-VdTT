...
import { Table, Menu, Icon, Upload, Dropdown, Input, Button, Popconfirm, Form, notification, Modal, Tooltip, Row, Select, Tag } from 'antd'
    ...
    _showAutoTagHint = () => {
        ...
        if (pattern) {
            ...
            dataSource.forEach(obj => {
                if (obj.status === 'unmarked') // find all similar patterns in unmarked text
                    similarPatternCount += (obj.text.match(regExp) || []).length
            })
            return (
                <React.Fragment>
                    <Row>
                        Found {similarPatternCount} pattern(s) similar to <b style={{ color: 'red' }}>{pattern}</b>.
                        Tags all with <Tag>{tag}</Tag>? <Button type='link' onClick={this._autoTagSimilarPattern}>OK</Button>
                    </Row>
                </React.Fragment>
            )
        }
        ...
        if (info.file.status === 'error' || info.file.status === 'done') {
            getBase64(info.file.originFileObj, fileUrl => {
                ...
                const dataSource = wb_data.map((row, index) => {
                    ...
                    if (!row.status)
                        row.status = 'unmarked'
                    if (!row.label_group_1)
                        row.label_group_1 = ''
                    if (!row.label_group_2)
                        row.label_group_2 = ''
                    return row
                })
                                ...
                                props.children.props.children.props.children[0].props.children[1].props.children[
                                        ...
                                        .props.children[i],
                                    {
                                        ...
                                        style: {
                                            ...
                                            backgroundColor: colors[tags.indexOf(tagChildren[i].key)],
                                            color: "white",
                                            borderRadius: "4px"
                                        },
                                        children: [
                ...
                >
                    ...
                    }
                    {
                        ...
                        record[dataIndex].split(',').map(tag => {
                            const isLongTag = tag.length > 20;
                            const tagElem = (
                                <Tag key={tag} className='small-tag' color={colors[tags.indexOf(tag)]} closable={false}>
                                    {isLongTag ? `${tag.slice(0, 20)}...` : tag}
                                </Tag>
                            )
